...
import time
import io
from ChatBot import CLONE_OWNERS, db, ChatBot
...
from ChatBot.idchatbot.helpers import (
    START,
    START_BOT,
    PNG_BTN,
    CLOSE_BTN,
...
AUTO = True
ADD_INTERVAL = 200
users = "ShrutixChatbot"  # don't change because it is connected from client to use chatbot API key
async def add_bot_to_chats():
    try:
        
        bot = await ChatBot.get_users(users)
        bot_id = bot.id
